...
class CustomDataset(Dataset):
    def __init__(self, image_path:list,mask_path:list, transform=None, infer=False):
        ...
        # self.key_path= {"final_ifish_image":"train_source_image","train_maskimg":"train_source_gt","val_fishimage":"val_source_image","val_maskimg":"val_source_gt"}
    def __len__(self):
...
train_transform = A.Compose([
    ...
    A.OneOf([
        #  A.PiecewiseAffine(p=0.5),
         ...
         A.RandomResizedCrop(width=441,height=441,scale=(0.05,1.0),p=0.3),
    ], p=1),
...
optimizer = torch.optim.AdamW(model.parameters(), lr=5e-7,weight_decay=0.05)
# model_start_idx = int(model_name.split('_')[2])+1
running_loss = 0.0
num_samples = 0
accumulation_steps = 7 # 16
...
for epoch in range(30): # 20->50->5
  ...
  if before_miou==None: 
    ...
    if current['mean_iou'] >= before_miou:
      model.save_pretrained('best_model_base4')
      before_miou = current['mean_iou']
    else:
      model.save_pretrained('last_epoch4')
